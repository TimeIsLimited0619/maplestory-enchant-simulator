...
Object.assign(PotentialModule, {
  ...
  isUniOverlayOpen() {
    return !document.getElementById('ptUniOverlay')?.classList.contains('hidden');
  },

  openUniOverlay(cube) {
    const lines = this.itemData?.potential?.lines;
    if (!lines?.length) {
    ...
    const overlay = document.getElementById('ptUniOverlay');
    if (overlay) {
  ...
  closeUniOverlay() {
    ...
    if (!overlay || overlay.classList.contains('hidden')) {
      this.resetUniOverlayState();
      this.uniClosing = false;
      this.updateUI();
      this.updateResetButtonState();
      return;
    }
    if (this.uniClosing) return;

    this.uniClosing = true;
  ...
  resetUniLine() {
    ...
    const rankBeforeRoll = this.itemData.potential.rank;
    const rolled = rollUnionLine(this.itemData, this.itemData.potential, this.uniSelectedLineIndex);
    if (!rolled) {
      return addLog('⚠️ 結合方塊機率資料載入失敗。', 'log-fail');
    }

    const lines = (this.uniBaseLines || this.itemData.potential.lines).map((line) => ({ ...line }));
    lines[this.uniSelectedLineIndex] = { ...rolled };
    const previewAtk = rollUnionPreviewAtkPow(this.itemData.potential);
    ...
    this.updateUI();
    this.renderUniOverlay();
    addLog('🔮 結合方塊已重新設定選擇的潛在能力。', 'log-success');
  ...
  getUniDisplayLines() {
    if (this.uniPhase === 'preview') {
      return (this.itemData?.potential?.lines || []).map((line) => ({ ...line }));
    }

    const base = this.uniBaseLines || this.itemData?.potential?.lines || [];
  ...
  renderUniLineRow(line, index, { selected, preview }) {
    ...
    const icon = UNI_UI.icon[line.rank] || rank.statIcon;
    const classes = [
      'pt-uni-line',
  ...
  renderUniOverlay() {
    ...
    const title = document.getElementById('ptUniTitle');
    const lineList = document.getElementById('ptUniLineList');
    const atkDisplay = document.getElementById('ptUniAtkPow');
    const countDisplay = document.getElementById('ptUniCubeCount');
    const confirmBox = document.getElementById('ptUniConfirmBox');
    const actionsBefore = document.getElementById('ptUniActionsBefore');
    const actionsAfter = document.getElementById('ptUniActionsAfter');
    const atkOutline = document.getElementById('ptUniAtkOutline');
    const choiceBox = document.querySelector('.pt-uni-choice-box');

    const rankId = this.itemData?.potential?.rank || 'rare';
    if (title) {
      title.style.backgroundImage = `url('${UNI_UI.title[rankId] || UNI_UI.title.rare}')`;
    }
    ...
    if (atkDisplay && atkOutline) {
      if (this.uniPhase === 'preview') {
        const previewAtk = this.uniPreviewAtkPow ?? this.itemData?.potential?.atkPow ?? 0;
    ...
    if (countDisplay && this.uniCubeId) {
      countDisplay.textContent = this.formatUniCubeCount(getPlayerCubeCount(this.uniCubeId));
    }
