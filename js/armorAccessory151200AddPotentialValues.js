/**
 * 防具與飾品 151~200 級附加潛能詞條數值（所有附加潛能方塊共用）
 * 來源：飾品_附加潛能屬性與數值對照表 1(151-200級_附加潛能總覽).csv
 * 機率沿用 event8422；此檔僅定義洗出後的數值／文案。
 */

const ARMOR_ACC_151200_ADDPOT_LEVEL_MIN = 151;
const ARMOR_ACC_151200_ADDPOT_LEVEL_MAX = 200;

const ARMOR_ACC_151200_ADDPOT_PERCENT = {
  rare: {
    stat: '3%',
    maxHpMp: '3%',
    def: '3%'
...
const ARMOR_ACC_151200_ADDPOT_FLAT = {
  rare: {
    main: 11,
    atk: 11,
    maxHpMp: 125,
...
const ARMOR_ACC_151200_ADDPOT_HP_RECOVERY = {
  unique: '20%',
  legendary: '30%'
};

const ARMOR_ACC_151200_ADDPOT_LEVEL_SCALE = {
  unique: 1,
  legendary: 2
};

function isArmorAccessory151200AddPotItem(item) {
  if (!item) return false;
  if (item.mainType !== EQUIP_TYPE.ARMOR && item.mainType !== EQUIP_TYPE.ACCESSORY) return false;
  const level = Number(item.reqLevel) || 0;
  return level >= ARMOR_ACC_151200_ADDPOT_LEVEL_MIN && level <= ARMOR_ACC_151200_ADDPOT_LEVEL_MAX;
}

function isArmorAccessory151200AddPotentialContext(context = {}) {
  if ((context.eventId || 8422) !== 8422) return false;
  return isArmorAccessory151200AddPotItem(context.item);
}

function getArmorAcc151200AddPotSlotKind(item) {
  if (!item) return null;
...
function getAddPotEntryScope(context) {
  return context.group?.entries?.[context.entryIndex]?.scope || null;
}

function classifyArmorAccessory151200AddPotStat(statName) {
...
function armorAccessory151200AddPotLabelForStat(statName) {
...
function formatArmorAccessory151200AddPotentialStatValue(statName, internalRank, context = {}) {
  if (!isArmorAccessory151200AddPotentialContext(context)) return null;

  const kind = classifyArmorAccessory151200AddPotStat(statName);
  if (!kind || kind === 'levelScale' || kind === 'hpRecovery' || kind === 'cooldownReduce') return null;

  const item = context.item;
  const slot = getArmorAcc151200AddPotSlotKind(item);
  const pct = ARMOR_ACC_151200_ADDPOT_PERCENT[internalRank];
  const flat = ARMOR_ACC_151200_ADDPOT_FLAT[internalRank];
  ...
  switch (kind) {
    ...
    case 'defPercent':
      return pct?.def || null;
    case 'allStatFlat':
      return internalRank === 'rare' ? String(ARMOR_ACC_151200_ADDPOT_FLAT.rare.allStat) : null;
    case 'allStatPercent':
    ...
    case 'critDamage': {
      ...
      if (slot === 'glove') {
        const scope = getAddPotEntryScope(context);
...
function parseArmorAccessory151200AddPotentialStat(statName, internalRank, context = {}) {
  if (!isArmorAccessory151200AddPotentialContext(context)) return null;
  ...
  if (kind === 'hpRecovery') {
    const percent = ARMOR_ACC_151200_ADDPOT_HP_RECOVERY[internalRank];
    if (!percent) return null;
    return { label: `HP恢復道具及恢復技能效果增加 ${percent}`, value: '' };
  }

  if (kind === 'levelScale') {
    const perStep = ARMOR_ACC_151200_ADDPOT_LEVEL_SCALE[internalRank];
    if (perStep == null) return null;
    return { label: statName, value: String(perStep) };
  }

  const value = formatArmorAccessory151200AddPotentialStatValue(statName, internalRank, context);
  if (value == null) return null;

  return {
    label: armorAccessory151200AddPotLabelForStat(statName),
    value
  };
}
